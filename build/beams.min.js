...
(function(){Beams.Style=class{constructor(options){options=options||{},this._data={loaderColor:"white",loaderScale:3},Object.assign(this._data,this.defaults()),Object.assign(this._data,options)}defaults(){throw Error("defaults() not implemented!")}},Beams.style_interceptor={get(obj,prop){return obj._data[prop]}}})();
(function(){class Interface extends Beams.EventInterface{constructor(options){options=options||{},super(options),this._loader=null,this._loader_g=null,this._loading=options.loading||!1,this._margins=Beams.point(options.margins||[0,0]),this._snap=Snap(),this._style=new(this.StyleClass())(options.style||{}),this._loading&&this.update_loader()}StyleClass(){throw Error("StyleClass() not implemented!")}get snap(){return this._snap}get node(){return this._snap.node}get style(){return new Proxy(this._style,Beams.style_interceptor)}get margins(){return this._margins}set margins(margins){this._margins=Beams.point(margins),this.render()}get loading(){return this._loading}set loading(isLoading){this._loading=isLoading,this.update_loader(),this.emit("state:loading",this._loading)}get bbox(){return{x:this.margins.x,y:this.margins.y,w:this.node.clientWidth-2*this.margins.x,h:this.node.clientHeight-2*this.margins.y,center(){return Beams.point(this.x+this.w/2,this.y+this.h/2)}}}inject(element){if(element){var place=element;if("string"==typeof element){var id=element.replace(/^#/,"");place=document.getElementById(id)}place.appendChild(this.node),this.render()}}update_loader(){if(!this.loading)return void(this._loader_g&&this._loader_g.attr({display:"none"}));const box=this._snap.getBBox();this._loader||(this._loader_g=this.snap.g(),this._loader=this._loader_g.path(),this._loader.attr({d:`M73,50c0-12.7-10.3-23-23-23S27,37.3,27,50M30.9,50c0-10.5,8.5-19.1,19.1-19.1S69.1,39.5,69.1,50`,fill:this.style.loaderColor}),this._loader.node.innerHTML=`
                <animateTransform 
