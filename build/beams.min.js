...
(function(){class Interface extends Beams.EventInterface{constructor(options){options=options||{},super(options),this._loader=null,this._loading=options.loading||!1,this._margins=Beams.point(options.margins||[0,0]),this._snap=Snap(),this._loading&&this.update_loader()}get snap(){return this._snap}get node(){return this._snap.node}get margins(){return this._margins}set margins(margins){this._margins=Beams.point(margins),this.render()}get loading(){return this._loading}set loading(isLoading){this._loading=isLoading,this.update_loader(),this.emit("state:loading",this._loading)}get bbox(){return{x:this.margins.x,y:this.margins.y,w:this.node.clientWidth-2*this.margins.x,h:this.node.clientHeight-2*this.margins.y,center(){return Beams.point(this.x+this.w/2,this.y+this.h/2)}}}inject(element){if(element){var place=element;if("string"==typeof element){var id=element.replace(/^#/,"");place=document.getElementById(id)}place.appendChild(this.node),this.render()}}update_loader(){if(!this.loading)return void(this._loader&&this._loader.attr({display:"none"}));this._loader||(this._loader=this.snap.path(),this._loader.attr({d:"M73,50c0-12.7-10.3-23-23-23S27,37.3,27,50 M30.9,50c0-10.5,8.5-19.1,19.1-19.1S69.1,39.5,69.1,50",fill:"#fff"}),this._loader.node.innerHTML=`
                <animateTransform 
                 ...
                 from="0 50 50"
                 to="360 50 50" 
                 repeatCount="indefinite" />`);const box=this._snap.getBBox(),lbox=this._loader.getBBox();this._loader.paper.append(this._loader),this._loader.attr({transform:`t${box.cx-lbox.w},${box.cy-2*lbox.h}`,display:void 0})}render(){}}Beams.Interface=Interface})();
